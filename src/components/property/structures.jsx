...
export const StructureUpgrades = ({ setItemDetails, purchaseItem, deleteItem, newUnlocks, isMobile }) => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            sendData('query-furnitures-data', { filterId: 'structure' });
        }, 100);
        return () => {
    ...
    const setHideMaxed = (hideMaxed) => {
        sendData('set-furniture-hide-maxed', { filterId: 'structure', hideMaxed: hideMaxed });
    }

    const setShowHidden = (showHidden) => {
        sendData('set-furniture-show-hidden', { filterId: 'structure', showHidden })
    }
    ...
    const toggleAutopurchase = useCallback((id, flag) => {
        sendData('set-furniture-autopurchase', { id, flag, filterId: 'structure' })
    })

    const toggleHiddenItem = useCallback((id, flag) => {
        sendData('toggle-furniture-hidden', { filterId: 'structure', id, flag });
    })


    const handlePinToggle = (id, newFlag) => {
        sendData('toggle-property-custom-filter-pinned', { id, flag: newFlag, filterId: 'structure' });
    };


    const setActionsFilter = (filterId) => {
        sendData('apply-property-custom-filter', { id: filterId, filterId: 'structure' })
    }

    const handleEditFilter = (id) => {
        // знаходите фільтр, відкриваєте форму редагування
        // наприклад:
        const filterData = structuresData.customFilters[id];
        setEditingCustomFilter({ ...filterData });
    };

    const handleDeleteFilter = (id) => {
        sendData('delete-property-custom-filter', { id, filterId: 'structure' });
    };
    ...
    return (<DragDropContext onDragEnd={onDragEnd}><div className={'furniture-wrap'}>
        <div className={'head'}>
            <TippyWrapper content={<div className={'hint-popup'}>
                <BreakDown breakDown={structuresData.space.breakDown} />
            </div> }>
                <div className={'space-item'}>
                    <RawResource id={'living_space'} name={'Living Space'} />
                    <span className={`slots-amount ${structuresData.space.total > 0 ? 'slots-available' : 'slots-unavailable'}`}>{formatInt(Math.floor(structuresData.space.total))}/{formatInt(structuresData.space.max)}</span>
                </div>
            </TippyWrapper>
            <div className={'filters'}>
                <label>
                    <SearchField
                        placeholder={'Search'}
                        value={structuresData.searchData}
                ...
                <label>
                    Hide maxed
                    <input type={'checkbox'} checked={structuresData.hideMaxed} onChange={e => setHideMaxed(!structuresData.hideMaxed)}/>
                </label>
                <label>
                    Show Hidden
                    <input type={'checkbox'} checked={structuresData.showHidden} onChange={e => setShowHidden(!structuresData.showHidden)}/>
                </label>
            </div>
        </div>
        <div className={'categories flex-container sub-heading'}>
            <ul className={'menu'}>
                {structuresData.propertyCategories.filter(one => one.isPinned || one.isSelected).map(category => (<li key={category.id} className={`category ${category.isSelected ? 'active' : ''}`} onClick={() => setActionsFilter(category.id)}>
                    ...
                    {isCustomFilterOpened ? (<div className={'custom-filter-edit-wrap'}>
                        {editingCustomFilter ? (
                                <CustomFilter
                                    ...
                                    onSave={(data) => {
                                        sendData('save-property-custom-filter', {...data, filterId: 'structure'});
                                        setEditingCustomFilter(null);
                                    }}
                                />)
                            : (<CustomFiltersList
                                filterOrder={structuresData.customFiltersOrder}
                                filters={structuresData.customFilters}
        ...
        <div className={'furnitures-cat'}>
            <PerfectScrollbar>
                <div className={'flex-container'}>
                    {structuresData.available.map(structure => <NewNotificationWrap key={structure.id} id={structure.id} className={'narrow-wrapper'} isNew={newUnlocks?.[structuresData.selectedCategory]?.items?.[structure.id]?.hasNew}>
                        <ItemCard key={structure.id} {...structure} onFlash={handleFlash} onPurchase={purchaseItem} onShowDetails={setItemDetails} onDelete={deleteItem} toggleAutopurchase={toggleAutopurchase} isAutomationUnlocked={structuresData.isAutomationUnlocked} isMobile={isMobile} toggleHiddenItem={toggleHiddenItem}/>
                    </NewNotificationWrap>)}
        ...
        <div className={'bottom'}>
            <div className={'buttons'}>
                <div className={'leftwise'}>
                    ...
                    <TippyWrapper content={<div className={'hint-popup'}>{isHidden ? 'Show Structure' : 'Hide Structure'}</div> }>
